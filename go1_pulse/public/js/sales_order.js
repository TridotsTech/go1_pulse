...
frappe.ui.form.on('Sales Order', {
    refresh: function(frm){
        ...
        if (frm.doc.docstatus === 1){
            ...
            // Open
            if (frm.doc.custom_so_status != 'Closed' && frm.doc.custom_so_status != "On Hold"){
                frm.add_custom_button(__('Hold'), () => {make_so_on_hold()},"SO Status")
            }
            

            // On-Hold
            if (frm.doc.custom_so_status == "On Hold" ){
                setTimeout(function(){
                    frm.remove_custom_button('Update Item')
                },100)
                frm.add_custom_button(__('Open'), () => {open_so()},"SO Status")
                frm.add_custom_button(__('Close'), () => {make_so_closed()},"SO Status")
            }

            // Not Closed
            if (frm.doc.custom_so_status != "Closed"){
                frm.add_custom_button(__('Close'), () => {make_so_closed()},"SO Status")
            }
            ...
            if (frm.doc.custom_so_status == "Closed"){
                setTimeout(function(){
                    ...
                    frm.add_custom_button(__('Open'), () => {open_so()})
                    frm.change_custom_button_type('Open',null,'primary')
                },100)
            }

            // Modification Approval
            if (frm.doc.sent_for_approval){
                frappe.db.get_single_value('SO Approver Settings', 'modification_approver').then(role => {
                frappe.call({
                    method:"revenue.apiphase2.get_modification_approver_roles",
                    args:{},
                    callback:function(role){
                        frm.clear_custom_buttons()
                        if(frappe.user.has_role(`${role.message}`)){
                            frm.add_custom_button(__('Approve'), () => {
                                console.log("Approver, Approved the modifications!");
                                frm.set_value('custom_so_status',"Open")
                                frm.set_value('sent_for_approval',0)
                                frm.save('Update')
                            })
        ...
        cur_frm.fields_dict.items.grid.get_field('end_customer').get_query = function(frm, cdt, cdn){
            let row= locals[cdt][cdn];
            let c_list=[];
            frappe.call({
                method:"revenue.api.get_customer_list",
        ...
        function open_so(){
            frappe.confirm('Are you sure you want to Open?',
                () => {
                    frm.set_value('custom_so_status',"Open")
                    frm.save("Update")
                }, () => {
        ...
        function make_so_on_hold(){
            frappe.confirm('Are you sure you want to put On-Hold?',
                () => {
                    frm.set_value('custom_so_status',"On Hold")
                    frm.save("Update")
                }, () => {
                })
        }

        function make_so_closed(){
            frappe.confirm('Are you sure you want to Close?',
                () => {
                    frappe.call({
                        method:"go1_pulse.api.validate_before_closing",
                        args:{
                            docName:frm.doc.name
                        },
                        callback:(r)=>{
                            frm.set_value('custom_so_status',"Closed")
                            frm.save("Update")
                        }
                    })
                }, () => {
