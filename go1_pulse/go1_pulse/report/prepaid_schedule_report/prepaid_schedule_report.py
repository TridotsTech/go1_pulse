...
from frappe.utils import add_days, date_diff,formatdate, flt,cint, today, get_first_day, get_last_day, rounded,getdate,add_months
from erpnext.accounts.report.financial_statements import get_period_list as _get_period_list,get_label
...
class Deferred_Item(object):
	...
	def report_data(self):
		...
		ret_data["item_name"] = self.name
		ret_data["base_net_amount"] = self.base_net_amount
		ret_data["deferred_account"] = self.deferred_account
		doctype = "Sales Invoice Item" if frappe.db.exists("Sales Invoice Item", self.name) else "Purchase Invoice Item"
		item = frappe.get_doc(doctype, self.name) or frappe.get_doc(doctype, self.name)
		doc = frappe.get_doc(item.parenttype,  item.parent)
		amount, base_amount = get_already_booked_amount(doc, item)
		ret_data["unamortised_balance_amount"] = rounded(self.base_net_amount - base_amount, 2)
		return ret_data
	...
	def simulate_future_posting(self):
		...
		if self.service_start_date != self.service_end_date:
			if add_days(self.last_entry_date, 1) < self.period_list[-1].to_date:
				self.estimate_for_period_list = get_custom_period_list(
...
class Deferred_Revenue_and_Expense_Report(object):
	...
	def get_period_list(self):
		...
		self.period_list = get_custom_period_list(
			self.filters.from_fiscal_year,
			self.filters.to_fiscal_year,
	...
	def get_columns(self):
		...
		columns.append({"label": _("Party name"),"fieldname" : "party_name", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("Start Date"), "fieldname" : "service_start_date", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("End Date"),"fieldname" : "service_end_date", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("PL Expense ledger"), "fieldname": "deferred_account", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("Base Amount"), "fieldname" : "base_net_amount", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("Unamortised Balance Amount"), "fieldname" : "unamortised_balance_amount", "fieldtype": "Data", "read_only": 1})
		columns.append({"label": _("Item_name"), "fieldname" : "item_name", "fieldtype": "Data", "read_only": 1, "hidden":1})
...
def execute(filters=None):
	...
	datas = report.generate_report_data()
	
	for data in enumerate(datas):
		
		if not filters.type == "Expense":
			data['party_name'] = frappe.get_value("Sales Invoice", data.get("name"), "customer" )
   
		if filters.type == "Expense":
				data['party_name'] = frappe.get_value("Purchase Invoice", data.get("name"), "supplier" )
	message = []
	chart = report.prepare_chart()

	return columns, datas, message, chart

def get_custom_period_list(
	from_fiscal_year,
	to_fiscal_year,
	period_start_date,
	period_end_date,
	...
	from erpnext.accounts.report.financial_statements import get_months,validate_dates,get_fiscal_year_data,validate_fiscal_year
