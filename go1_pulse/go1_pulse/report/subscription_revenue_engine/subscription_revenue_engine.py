...
from go1_pulse.queries import subscription_revenue_engine_query
...
class Deferred_Item(object):
	...
	def __init__(self, item, inv, gle_entries):
		...
		if gle_entries:
			...
			print("gl entries ", self.gle_entries)
...
class Deferred_Revenue_and_Expense_Report(object):
	...
	def get_invoices(self):
		...
		if self.filters.type == "Revenue":
			inv = qb.DocType("Sales Order")
			inv_item = qb.DocType("Sales Order Item")
   
			query= subscription_revenue_engine_query(period_list=self.period_list)

		self.invoices = frappe.db.sql(query=query, as_dict=True)
